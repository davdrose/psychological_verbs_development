...
- **exp2** — control response columns come from the main `-responses.csv`; the
  experimental responses are merged in from
  `data/experiment2/adults/raw/drums_experimental_responses.csv`.
- **exp1 / exp3** — uniform response columns (`{scenario}_{question}`).
- exp1/exp3 drop duplicate `workerid` submissions (keep first) and print which.
...
- Condition is inferred per child (exp2: causal vs. psych verbs; exp1: physical vs.
  mental); exp2 children who saw all four verbs are excluded.
